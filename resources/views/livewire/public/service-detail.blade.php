<div class="min-h-screen bg-[#070708] text-[#e5e5e5] antialiased selection:bg-purple-500 selection:text-white overflow-x-hidden">
    ...
    @section('meta_title', $service->title . ' | Sofyan Abdul Fatih')
    @section('meta_description', Str::limit(strip_tags($service->description), 160))
    @section('meta_keywords', 'Service, ' . $service->category . ', ' . $service->title . ', Sofyan Abdul Fatih')
    ...
    <div class="fixed bottom-8 left-6 md:left-10 z-[100]">
        <button type="button" x-on:click="window.history.back()"
            class="group w-12 h-12 rounded-full border border-white/10 flex items-center justify-center
                   hover:bg-purple-500 hover:border-purple-500 transition-all duration-400">
    ...
    <header class="max-w-[1400px] mx-auto px-5 md:px-10 pt-32 pb-10">
        <div class="flex items-center gap-4 mb-6">
            ...
            <span class="font-mono text-[9px] text-purple-500/70 tracking-[0.3em] uppercase">
        ...
        <h1 class="text-[clamp(3rem,9vw,8rem)] font-black tracking-[-0.04em] leading-[0.85] uppercase"
            ...
            {{ $service->title }}<span class="text-purple-500">.</span>
        </h1>
    </header>
    ...
    <div class="max-w-[1400px] mx-auto px-5 md:px-10 pb-32">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3">
            ...
            <div class="lg:col-span-4 border border-white/[0.06] bg-[#0d0d0e] p-8 flex flex-col justify-between">
                <div>
                    <p class="text-[9px] font-bold uppercase tracking-[0.4em] text-purple-400/60 mb-4">Overview</p>
                ...
                <a href="{{ route('home') }}#contact"
                    class="mt-8 inline-flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.3em]
                           text-purple-400 hover:text-white transition-colors duration-200 group/link">
            ...
            {{-- [5] CTA card --}}
            <div class="lg:col-span-3 border border-purple-500/20 bg-purple-500/[0.04] p-8 flex flex-col justify-between">
                <div class="relative flex h-2 w-2">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                </div>
                <div>
                    <p class="text-[9px] font-bold uppercase tracking-[0.4em] text-purple-400/60 mb-2">Ready to start?</p>
                    ...
                    <a href="{{ route('home') }}#contact"
                        class="inline-flex items-center justify-center w-full py-3 bg-purple-500 text-white
                               text-[9px] font-bold uppercase tracking-[0.3em]
                               hover:bg-purple-400 active:scale-[0.98] transition-all duration-200">
                        Book Consultation ↗
                    </a>
                </div>
            </div>
                    ...
                    @foreach ($features as $i => $feature)
                        <div class="group/f flex items-center justify-between py-4">
                            <div class="flex items-center gap-3">
                                <span class="text-[8px] font-mono text-purple-500/40 flex-shrink-0">{{ sprintf('%02d', $i + 1) }}</span>
                                <span class="text-xs font-bold text-white/50 group-hover/f:text-white transition-colors duration-200">{{ $feature }}</span>
                            </div>
                            <div class="w-1.5 h-1.5 rounded-full bg-purple-500/20 group-hover/f:bg-purple-500 transition-colors duration-200 flex-shrink-0"></div>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="lg:col-span-4 border border-white/[0.06] bg-[#0d0d0e] p-8 flex flex-col justify-between">
                <div>
                    <div class="text-purple-500/20 text-5xl font-black leading-none mb-4"
                ...
                <div class="mt-8 pt-6 border-t border-white/[0.05]">
                    <div class="flex items-center gap-3 mb-1">
                        <div class="w-5 h-px bg-purple-500"></div>
                        <span class="text-[8px] font-bold uppercase tracking-[0.3em] text-white/30">Sofyan Abdul Fatih</span>
                    </div>
                    <span class="text-[8px] font-mono text-purple-400/40 tracking-widest">Creative Director</span>
                </div>
            </div>
            ...
            <div class="sm:col-span-2 lg:col-span-12 border border-white/[0.06] bg-[#0d0d0e] p-8 md:p-10">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    @foreach ([
                        ...
                        <div class="group/p border border-white/[0.06] bg-[#111] p-6 hover:border-purple-500/30 transition-colors duration-300">
                            <p class="font-mono text-[9px] text-purple-500/50 mb-4">{{ $num }}</p>
    ...
    <footer class="relative min-h-[65vh] flex flex-col items-center justify-center bg-white text-black
        ...
        <div class="relative z-10">
            ...
            <h2 class="text-5xl md:text-[8vw] font-black tracking-[-0.04em] leading-[0.9] mb-12"
                style="font-family:'Bebas Neue','Arial Black',sans-serif;">
                Ready to make<br/><span class="text-purple-500">an Impact?</span>
            </h2>
            <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
                <a href="{{ route('home') }}#contact"
                    ...
                    <div class="absolute inset-0 bg-purple-500 translate-y-full group-hover:translate-y-0 transition-transform duration-500"></div>
                </a>
                <a href="{{ route('home') }}#works"
                    ...
                    <span class="absolute top-full left-0 block group-hover:-translate-y-full transition-transform duration-500 text-purple-500">Explore Portfolio</span>
                </a>
            </div>
        </div>
    ...
    <style>
        ...
        .prose strong{color:#c084fc !important;}
