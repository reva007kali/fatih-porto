<div class="min-h-screen py-6 bg-[#0b0b0d]">
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-400 mt-2">Create a new landing page or website.</p>
        </div>
        ...
        <div class="bg-[#1c1c1e] border border-white/5 rounded-3xl overflow-hidden shadow-xl shadow-black/20">
            <div class="p-8 sm:p-10">
                <form wire:submit.prevent="store" class="space-y-6" x-data="{
                    updateContent() {
                        if (this.$refs.aceComponent) {
                            // Find the Ace editor component scope and trigger sync
                            // Or better, just rely on the 'change' event we set up in the editor
                        }
                    }
                }">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="slug"
                                class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Slug</label>
                        ...
                        <div>
                            <label for="tags"
                                class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Tags</label>
                        ...
                        <div>
                            <label for="color"
                                class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Theme
                                Color</label>
                            <div class="flex items-center gap-2">
                                <input type="color" wire:model="color"
                                    class="h-10 w-10 rounded-lg border border-white/10 cursor-pointer bg-[#0b0b0d] p-1">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Cover
                                Image</label>
                            <div class="flex items-center gap-4">
                                @if ($image)
                                    <div
                                        class="w-20 h-20 rounded-xl border border-white/10 overflow-hidden bg-black flex-shrink-0">
                                ...
                                <label class="flex-1 cursor-pointer">
                                    <div
                                        class="w-full bg-[#0b0b0d] border border-dashed border-white/10 rounded-xl px-4 py-3 text-center hover:bg-white/5 transition-colors">
                                        <span class="text-xs text-gray-400">Upload Cover</span>
                                        <input type="file" wire:model="image" class="hidden" accept="image/*">
                                    </div>
                                </label>
                            </div>
                            <div wire:loading wire:target="image" class="text-xs text-swiss-blue mt-2 font-bold">
                                Uploading...</div>
                            <x-input-error class="mt-2 text-xs text-red-500" :messages="$errors->get('image')" />
                        </div>

                        <div class="col-span-1 md:col-span-2" wire:ignore>
                            <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Website
                                Content (HTML/Blade)</label>

                            {{-- AI Generator --}}
                            <div class="mb-4 p-4 bg-white/5 rounded-xl border border-white/10">
                                <label
                                    class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">AI
                                    Generator</label>
                                <div class="flex gap-2">
                                    <input wire:model="aiPrompt" type="text"
                                        placeholder="Describe the section you want to generate (e.g. 'A hero section with a blue gradient background...')"
                                    ...
                                    <button type="button" @click="$dispatch('start-ai-generation')"
                            ...
                            <div x-data="{
                                ...
                                @start-ai-generation.window="startGeneration()" wire:ignore
