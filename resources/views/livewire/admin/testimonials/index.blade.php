<div class="min-h-screen py-6 bg-[#0b0b0d]">
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-400 mt-2">What your clients say about you.</p>
        </div>
            ...
            <div class="lg:col-span-1 order-2 lg:order-1">
                <div class="bg-[#1c1c1e] border border-white/5 rounded-3xl overflow-hidden shadow-xl shadow-black/20 sticky top-6">
                    <div class="p-6">
                        ...
                        <p class="text-xs text-gray-500 mb-6">Client feedback details.</p>

                        <form wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}" class="space-y-5">
                            <div>
                                <label for="name" class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Client Name</label>
                            ...
                            <div>
                                <label for="position" class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Position / Company</label>
                                <input wire:model="position" id="position" type="text" placeholder="CEO, Acme Inc."
                                    class="w-full bg-[#0b0b0d] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:border-swiss-blue focus:ring-1 focus:ring-swiss-blue transition-all outline-none"
                                    required>
                                <x-input-error class="mt-2 text-xs text-red-500" :messages="$errors->get('position')" />
                            </div>

                            <div>
                                <label for="content" class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Testimonial Content</label>
                            ...
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Avatar</label>
                                <div class="flex items-center gap-4">
                                    @if ($avatar)
                                        <div class="w-16 h-16 rounded-full border border-white/10 overflow-hidden bg-black flex-shrink-0">
                                    ...
                                    @else
                                        <div class="w-16 h-16 rounded-full border border-white/10 bg-[#0b0b0d] flex items-center justify-center flex-shrink-0">
                                    ...
                                    <label class="flex-1 cursor-pointer">
                                        <div class="w-full bg-[#0b0b0d] border border-dashed border-white/10 rounded-xl px-4 py-3 text-center hover:bg-white/5 transition-colors">
                                            <span class="text-xs text-gray-400">Upload Avatar</span>
                                            <input type="file" wire:model="avatar" class="hidden" accept="image/*">
                                        </div>
                                    </label>
                                </div>
                                <div wire:loading wire:target="avatar" class="text-xs text-swiss-blue mt-2 font-bold">Uploading...</div>
                                <x-input-error class="mt-2 text-xs text-red-500" :messages="$errors->get('avatar')" />
                            </div>
                            ...
                            <tbody class="divide-y divide-white/5">
                                @foreach($testimonials as $testimonial)
                                    <tr class="hover:bg-white/[0.02] transition-colors group">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-4">
                                                <div class="w-10 h-10 rounded-full bg-[#0b0b0d] border border-white/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
                                                    @if ($testimonial->avatar)
                                                        <img src="{{ asset('storage/' . $testimonial->avatar) }}" class="w-full h-full object-cover">
                                                    @else
                                                        <span class="text-xs font-bold text-gray-600">{{ substr($testimonial->name, 0, 1) }}</span>
                                                    @endif
                                                </div>
                                                <div>
                                                    <h4 class="font-bold text-white text-sm">{{ $testimonial->name }}</h4>
                                                    <p class="text-xs text-gray-500">{{ $testimonial->position }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <p class="text-xs text-gray-400 line-clamp-2 leading-relaxed italic">"{{ Str::limit($testimonial->content, 60) }}"</p>
                                        </td>
                                        <td class="px-6 py-4">
                                            <button wire:click="toggleStatus({{ $testimonial->id }})" 
                                                ...
                                                {{ $testimonial->is_active ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20' : 'bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20' }}">
